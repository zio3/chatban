...
- **3つの欄を使い分ける (#92)**: `summary`=いまどうなっているか(カードに出る。Reviewなら `実装完了 (commit xxx)` の一言) / `reason`=なぜこの担当か(パネルのみ) / `context`=経緯と検収エビデンスの詳細。reasonに進捗を書かない — 以前MCP側のツール契約にreasonの説明が無く、用途不明の文字列欄に見えたため進捗を書き込んで汚した。**ツール契約のdescriptionはエージェントにとってのUIラベル**であり、入口(チャット/MCP)で契約がズレると入口ごとに違う汚れ方をする
