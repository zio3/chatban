...
**AI HACK 2026 提出作品** (ハッカソン後は個人利用のツールとして続けています)。かんばんとチャットの二層UIで、カードの登録から完了までを会話で回すタスク管理ツールです。人間の確定が要るのは「ボードから退場させる操作」だけ — Doneへ至る経路は人間の検収ただ1本で、AIが通れる道はコード上に存在しません(再オープンで戻せます)。
...
**https://chatban.zio3.net**

共有の板です。ログインは要りません。**誰でも書き込めるので、好きに荒らして構いません** —
毎朝5時 (JST) にチュートリアル3枚の状態へ戻ります。

ただし**このデモでは MCP を塞いであります**。上に書いた「番号で指させる外部記憶」は
エージェントから MCP で繋いだときの話なので、**デモで体験できるのはそこまで行きません**。
デモで分かるのは「チャットで話すとカードが生える」「Done は人間の検収を通る」あたりまでです。
それが本題だと思ったら、手元で動かして `.mcp.json` で繋いでください (→ [起動方法](#起動方法) / [MCP連携](#mcp連携-ドッグフーディング))。

**期間限定です。**プリペイド残高が尽きた時点で終わりにする、と決めてあります (日付では切りません)。
尽きるとチャットだけが止まり、画面にその旨が出ます (板の操作は動いたままです)。
動いているコミットは [`/version.txt`](https://chatban.zio3.net/version.txt) で確認できます。

## コンセプト

- **カードの新規作成はチャット専用** — UIに作成フォームはありません。「ログイン画面のバグ直さないと」と話すとカードが生えます。スクショやPDFを貼って話すこともできます(原本は保存されず、AIが読んだ意味だけが記録に残る。ただし接続先がAnthropic形式=`apiStyle: "messages"` のときは添付を使えません — 添付欄も出ません)
...
- **Doneは陳列棚ではなくロスタイム** — 検収されたカードは「もう一回確認したい」ぶんだけ列に残り、次の検収でひとつの箱に畳まれ、24時間で板から降ります。消えるのは器のほうで、カードは番地と検索で引けます。**判断の記録はDone列ではなく経緯メモに残す** — 却下した(=やらないと決めた)カードは理由ごとそこに残ります
- **経緯が残る** — 「なぜそう決めたか」がカードの経緯メモに貯まり、要約にもチャットの検索にも効きます。経緯欄はどのツールにもありますが、埋まらないのが普通です。埋まるようになったのは、書く手間が消えたことより**必ず読む相手ができた**ことが大きい
- **会話が構造の代わり** — ステータスは固定4列 (どうしてもというときだけ任意レーンを最大2本足せます)。優先度・タグ・サブタスクはありません。「これ上にして」「#11は#14待ち」と言えば済むものは属性にしない
...
| 粒度 | Doneの3段 (バラバラ→箱→板から降りる) | 直近の完了だけを視界に置ける |
...
エージェントがカードを取って自己申告で閉じるキュー型では、どちらも起きにくくなります(人間が板を見に来る理由がなく、Doneを打つのもAI自身のため)。**人間の検収は安全のための足枷ではなく、記憶の品質を担保する装置**でもあった、というのが後から分かったことです。

## こんなことが言えます

```
「#10やりたい」                              → 経緯ごと引いて、続きから始められる
「気力がないので軽そうなカードから並べて」     → 意味で並び替える (キーでは作れない順序)
...
**以下は当時 (2026-08) の構成での記録**で、その後モデルも仕組みも変わっています (いまは OpenAI直 `gpt-5.6-luna`、Doneの要約カードは #200 で撤去し「畳んだ完了」の箱に置き換え)。数字は当時のものとして残します:
...
| Anthropic | https://console.anthropic.com/settings/keys | `~/.anthropic/apikey.txt` | プロンプトキャッシュが効きます。**添付(画像/PDF)は使えません**(下記)。**未検証**(手元のキーが失効していて通していません。モデルIDも候補です) |
...
- **キーは自分で置いてから頼む。** `~/.openai/apikey.txt` のようなファイルに1行で書いておき、設定には `apiKeyFile` でそのパスだけ書かせます。**あなたがキーを打ち込む必要が無くなる**のが効果です。ただし**隠せるわけではありません** — エージェントはそのファイルを読めるので、`cat` で中身を出せば会話の記録に載ります。「見せない」ではなく「**渡さなくて済む**」と考えてください
  ...
  同じフォルダに、自分だけが読める仮のファイルを作ってそこへ書き、
  中身を表示せず、空白を除いて空でないことを確かめてから本番の名前へ移して。
  成否にかかわらず、最後に仮のファイルを残さないで。
  ```

  貼り付け先を自分で開かずに済みます。**仮のファイルを本番と同じフォルダに作らせる**のがこの頼み方の要点です:

  - クリップボードが読めなかったとき、**前に置いたキーを失いません**(リダイレクトは書き込み先を空にしてから読むので、本番へ直接書かせると失敗と同時に中身が消えます)
  - 同じフォルダなら移動が単純な置き換えで済み、**リポジトリの中に仮ファイルが作られてコミットに混ざる**事故も避けられます。ただし移動は本番のファイルごと置き換えるので、**前の本番ファイルの権限は引き継がれません**。だから権限は**仮ファイルを作る時点で**絞らせます(あとから絞る形だと、書いている間と移した直後に緩いままの瞬間ができ、そこで失敗すると緩い本番ファイルだけが残ります)
  - 確かめるのは**空でないこと**だけです。`sk-` のような接頭辞で判定させるとプロバイダごとに違って**正しいキーを弾く**ほうが起きますし、中身を見せる理由もありません。**「空白を除いて」を付けるのは効きます** — 空白や改行だけのファイルはバイト数があるので素朴な確認を通ってしまい、置き換えたあとに起動時の検証が「空です」で落ちます(こちらは前後の空白を落として判定します)
...
プロジェクトごとに**SQLiteファイルが分かれます**。切り替えるとボード・チャット・前提情報がまとめて入れ替わり、カードの番号は各プロジェクトで `#1` から始まります。
...
ChatBan自体の開発カードは、ChatBanのボードで管理しました。Claude Codeが内蔵MCPサーバー経由でカードを拾い、実装し、Reviewに置き、人間が検収する — 「作っているツールで、作っているツールのカードを管理する」構図です。
...
このリポジトリは「作った機能」と同じくらい「**作らないと決めた機能**」を大事にしています。カスタムステータス、カテゴリ別チャット、登録承認UI、アプリ内音声入力、**経緯のベクトル検索(RAG)** — いずれも検討の末に理由つきで却下し、その判断自体がカードの経緯メモに残っています。**却下は要約しません** — 却下は理由が本体で、結論だけ残しても次に同じ案が出たときに止められないからです。
...
開発実録: 実質2日 (2026-08-09 〜 08-10)。この期間の全コミット・全会話がこのリポジトリと `backend/data/` に記録されています(会話ログは `search_cards` のキーワード検索から引けます。#262 で `query_log` の対象からは外しました)。LLM呼び出しの記録と「全ログExport」は #181 で撤去しました。
