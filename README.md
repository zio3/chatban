...
**AI HACK 2026 提出作品。** かんばんとチャットの二層UIで、タスクの登録・割り振り・完了までを会話で回すタスク管理ツールです。人間の確定が要るのは「取り返しのつかない操作」だけ — 割り振りはAIが確定まで進め(担当は後から変えられるので)、Doneへ至る経路は人間の検収ただ1本で、AIが通れる道はコード上に存在しません。
...
- **人間のゲートは入口ではなく出口** — 割り振りはAIが負荷と実績を根拠に判断して確定まで進めます(担当は後から変えられるので、承認を挟むより速い)。一方 Done は「報告→Review→人間の検収チェック」を必ず通り、チャットにもMCPにもDoneへ直行する経路はコードレベルで存在しません
...
```
「未割り当てのタスクをいい感じに振っといて」   → 負荷と実績を根拠に判断して割り振る
