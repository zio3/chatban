...
**AI HACK 2026 提出作品** (ハッカソン後は個人利用のツールとして続けています)。かんばんとチャットの二層UIで、タスクの登録から完了までを会話で回すタスク管理ツールです。人間の確定が要るのは「ボードから退場させる操作」だけ — Doneへ至る経路は人間の検収ただ1本で、AIが通れる道はコード上に存在しません(再オープンで戻せます)。
...
- **Doneは陳列棚ではなくロスタイム** — 検収されたタスクは「もう一回確認したい」ぶんだけ列に残り、次の検収でひとつの箱に畳まれ、24時間で板から降ります。消えるのは器のほうで、タスクは番地と検索で引けます。**判断の記録はDone列ではなく経緯メモに残す** — 却下した(=やらないと決めた)タスクは理由ごとそこに残ります
...
このリポジトリは「作った機能」と同じくらい「**作らないと決めた機能**」を大事にしています。カスタムステータス、カテゴリ別チャット、登録承認UI、アプリ内音声入力、**経緯のベクトル検索(RAG)** — いずれも検討の末に理由つきで却下し、その判断自体がタスクの経緯メモに残っています。**却下は要約しません** — 却下は理由が本体で、結論だけ残しても次に同じ案が出たときに止められないからです。
