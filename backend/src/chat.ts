...
export const PROJECT_CONTEXT_WRITE_DESCRIPTION =
  "プロジェクトの前提情報(全員共有、チャットのシステムプロンプトに常時含まれる)を上書き更新する。全文を渡すので、必ず先に読んで自分の変更をマージした全文にすること。完了の定義・列の意味(保留のときどこに置くかを含む)・稼働日など、そのプロジェクト固有の運用ルールが入っている";
...
export function buildSystemPrompt(cardFocus?: ReturnType<typeof getCard>, view?: string): string {
  ...
  return [
    ...
    "- 「終わりました」等の完了報告は status=review に置き、「Reviewに置いたので確認OKなら承認を」と一言返す。勝手に done にしない (doneは人間の検収済みという意味。確定したカードはしばらく個別に残り、次の検収のときに「畳んだ完了」の箱へ入る。箱は24時間ぶんをまとめたもので、板から降りても消えるのではなく archived になるだけ — 番号でも search_cards でも引ける)。",
    ...
    // #261: 前提情報から移してきた。**プロジェクトごとに変わらない製品の仕様**なので、
    // 盤ごとの文章に置いておくと、盤の数だけ同じことが書かれ、改修のたびに全部が腐る。
    "- カードの中身を見たいと言われたら、チャットに番号だけ(`193` / `#193`)を打てば開くと案内してよい。LLMを呼ばずに詳細が出る(文中の `#193` はクリックでも開き、アーカイブ済みのものも開ける)。",
