...
| リアルタイム | Socket.IO | ボード変更・ツール進捗の逐次配信 |
...
通常のリクエスト内のDB操作はこれで決まるので、引数で持ち回らない。
**別のプロジェクトを名指しするときだけ、明示的にスコープへ入り直す** —
`broadcastBoard(projectId)` は渡されたIDで `withProject` に入ってから payload を作る。
(#200 以前は、完了後の処理が非同期でリクエストの外に出ていたため**それ専用の入り直し**も要った。
そちらは畳み直しが同期になったので撤去済み)

## LLMモデル戦略

| 用途 | 既定のモデル (OpenAI直) | 理由 |
|---|---|---|
| 対話 (chat。suggest は #271 で撤去) | `gpt-5.6-luna` | 応答速度が生命線 + 自動キャッシュが実額に効く。#210 でツール呼び出しを実測して切り替え (15/15、gpt-5.4-mini と同点) |
...
- **DoneへのD&Dは禁止、Doneからの持ち出しも禁止**。Doneは「置き場」ではなく検収の結果で、
  ドラッグで到達できると退場ゲート (#57) を迂回できてしまうため
