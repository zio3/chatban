...
- **キーは自分で置いてから頼む。** `~/.openai/apikey.txt` のようなファイルに1行で書いておき、設定には `apiKeyFile` でそのパスだけ書かせます。**あなたがキーを打ち込む必要が無くなる**のが効果です。ただし**隠せるわけではありません** — エージェントはそのファイルを読めるので、`cat` で中身を出せば会話の記録に載ります。「見せない」ではなく「**渡さなくて済む**」と考えてください
  ...
  同じフォルダに仮のファイルを作ってそこへ書き、中身を表示せずに
  空白を除いて空でないことを確かめてから、本番の名前へ移して。
  権限は自分だけが読める状態にして。
  うまくいかなかった場合も含めて、最後に仮のファイルを残さないで。
  ```
  ...
  - 同じフォルダなら移動が単純な置き換えで済み、**リポジトリの中に仮ファイルが作られてコミットに混ざる**事故も避けられます。ただし**権限は引き継がれません** — 移動で本番のファイルごと置き換わるので、仮ファイル側を絞らせておく必要があります
