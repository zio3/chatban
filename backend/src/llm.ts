 ...
 *
 * 逆に**足す側**に入るのは、同じ `model` の truthy な `prev` で、現在の件数が
 * 先頭 round の `messageCount` より大きいとき。`rounds` が欠けている・空・先頭の
 * `messageCount` が無い場合は**先頭を 0 として扱う**ので、件数が正なら足す側になる。
 *
 * **ただし `rounds` が反復できない値だと、足す側に入ったあとで落ちる** — 下の
 * `[...prev.rounds]` が例外になり、`dumpRequest` の catch が拾って**その回のダンプごと
 * 書かれない** (足しも作り直しもしない、という第3の結果がある)。
 * 文字列は例外にならず、1文字ずつの配列として**壊れたまま書かれる**。
 *
