...
- **キーは自分で置いてから頼む。** `~/.openai/apikey.txt` のようなファイルに1行で書いておき、設定には `apiKeyFile` でそのパスだけ書かせます。**あなたがキーを打ち込む必要が無くなる**のが効果です。ただし**隠せるわけではありません** — エージェントはそのファイルを読めるので、`cat` で中身を出せば会話の記録に載ります。「見せない」ではなく「**渡さなくて済む**」と考えてください
  ...
  ファイルに置く作業そのものも、**手元のPCでCLIを動かしているなら**クリップボード経由で任せられます(任意の手段です)。発行画面でキーをコピーしてから、こう言います:

  ```
  クリップボードにAPIキーがあるので、~/.openai/apikey.txt に保存して。
  値は表示せず、Get-Clipboard の出力をそのままファイルへ流して。
  (macOS/Linux なら pbpaste / wl-paste / xclip -o)
  保存したらファイルの権限を自分だけに絞って。
  ```

  うまくいけば、**発行画面から一度もキーを目にせずに設置できます**。ただし**そうなる保証はありません**。成立するのは「CLIがあなたのデスクトップと同じOSで動いていてクリップボードに触れる」かつ「エージェントが値を表示せず、そのままリダイレクトした」ときだけです:

  - **リモートやコンテナで動くCLIには、あなたのクリップボードが見えません**(クラウド実行・WSL・SSH越し・headless)。Linuxも `wl-paste` / `xclip` が入っていないと使えません
  - エージェントが確認のために `Get-Clipboard` 単体を実行すれば、**値がツールの出力として記録に残ります**。だから「値は表示しないで」を頼み方に含めます
  - **クリップボードを空にしても消えきりません** — クリップボードマネージャや端末間の同期に残ります。**キーは漏れた前提で扱い、不安なら発行元で作り直す**のが確実です
  - macOS/Linux では**ファイルの権限を自分だけに絞ってください**(`chmod 600`)。umask次第で他のユーザーから読めます

  つまり**リスクを下げる手段であって、秘密を守る仕組みではありません**。
