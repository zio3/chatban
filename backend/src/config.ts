...
/** キーの続きとみなす範囲。**「何がキーか」ではなく「どこで終わるか」で決める。**
 *
 * 経緯が2段ある (どちらもCodexレビュー指摘):
 *
 * 1. 最初は `[A-Za-z0-9_*.-]` と**マスク文字を列挙**していた。**上流が何でマスクするかは
 *    予想できない** — 実物のOpenAIは ASCII の `*` だが、`…`(Unicode省略記号) や `xxxx` も
 *    ありうる。列挙するとそこで一致が止まり、**後ろに続く末尾数文字が残る** (`***…IgAA`)
 * 2. 次に「空白まで」(`\S*`) にしたら、今度は**伏せすぎた**。Messages経路は上流のJSONを
 *    そのまま本文に載せるので、`{"error":{"message":"invalid key sk-...","type":"..."},
 *    "request_id":"req_123"}` のように**空白が1つも無い**。キーが途中のフィールドにあると
 *    末尾まで飲み込み、`type` や `request_id` という**障害の手がかりまで消えた**
 *
 * そこで**構造の区切りで止める**。キーに現れないが JSON や文章の区切りには必ず現れる文字
 * (引用符・カンマ・閉じ括弧・セミコロン・空白) を終端にする。マスク文字が何であっても
 * 通り抜けられるうえ、値の外側は残る */
const KEY_TAIL = "[^\\s\"',;}\\])>]*";
...
/** 伏字に置き換える。**末尾に食い込んだ記号は返す。**
 *
 * KEY_TAIL は構造の区切り (引用符・カンマ・閉じ括弧・空白) で止まるので、JSONの `"}` までは
 * 飲み込まない。それでも**文末の句点やコロンは区切りに入っていない**ので、
 * `provided: sk-xxx. You can find` の `.` は一致に含まれる — そのままだと文が繋がって読めなくなる
 * (最初の版はユニットテストが拾った)。消すのは値だけにして、周りは残す。
 *
 * KEY_TAIL 側の終端を増やすとここの出番は減るが、**キーに現れうる文字を区切りにはできない**
 * (`.` はキーに入りうる)。だから「取りすぎたぶんを返す」役はこちらに残す */
...
export function redactSecrets(text: string, secrets: (string | undefined | null)[]): string {
  ...
  for (const s of secrets) {
    ...
    if (s.length >= 8) {
      out = out.replace(new RegExp(`${escapeRegExp(s.slice(0, 8))}${KEY_TAIL}`, "g"), mask);
    }
  }
  // 設定に無いキーの保険。**マスクが始まる前の文字数を条件にしない。**
  // 「12文字以上のキーらしい並び」→「8文字以上」と緩めたが、どちらも
  // `sk-abc***…` (マスクが4文字目から始まる) を取りこぼしていた — 救えると
  // コメントに書きながら救えていなかった (Codexレビュー指摘)。
  // `sk-` の後ろに6文字以上「区切りでない何か」が続けば伏せる。
  // 伏せすぎ側の危険は小さい: `sk-` で始まる無害な識別子は、この本文 (上流のエラー) に出てこない
  return out.replace(new RegExp(`\\bsk-[^\\s"',;}\\])>]{6,}`, "g"), mask);
}
