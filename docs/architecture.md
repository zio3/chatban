...
リクエストの間じゅう「どのプロジェクトを触っているか」が引き回されるので、
DB操作もSocket.IOの配信先roomも、引数で持ち回らずに決まる。
(#200 以前は完了後の処理が非同期でリクエストの外に出ていたため、呼ばれた時点のIDを捕まえて
入り直す必要があった。**いまは畳み直しも同期で完結する**ので、その入り直しは無い)
