...
- **ボードのフィルタもLLMに任せる (#91/#103/#106と同じ形)**: いまフィルタは担当者だけで、「締切が近いものだけ」のような絞り込みができない。**LLMにIDの集合を返させて、それで絞る**。`query_log` で `live_tasks` を引かせて `set_filter` に渡すだけなので、**新しい安全境界は要らない** (SQL窓口は readonly + 許可リスト #168 で既に守ってある)。WHERE句を受け取る案もあるが、#91 が「ソートキーを渡す方式を作りかけて捨てた」のと同じ理由で採らない —「重要そうな順」がキーで書けないのと同様に、「この件に関係しそうなものだけ」はWHEREで書けない。
  ...
  - **担当フィルタはこの定型ショートカットに格下げする。**述語ベース(動的)とID列ベース(静的)の二層を持たず、**押した瞬間にIDを確定させる**。挙動は変わる (絞った後に増えた同担当のタスクが見えるようになる) が、上のルールで一貫するほうを取る。判定ロジックも1種類になる
