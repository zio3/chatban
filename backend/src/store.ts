...
`);
  ...
  //
  // #232: 古い名前 (done_tasks / live_tasks) を落とす行がここに在ったが、全DBを移行し切ったので
  // 相手がいなくなった。旧スキーマを入口で止める番人も #239 で撤去した (バックアップから
  // 古いDBを戻したときは `scripts/migrate-cards.mjs` が空の cards 越しでも救う)
  db.exec(`
