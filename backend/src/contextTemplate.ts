...
export const CONTEXT_TEMPLATE = [
  ...
  // #266: 節ごと落とした「できなかったとき・やらないとき」から、**この1行だけ残した**。
  // 却下・誤登録の扱いは製品共通で契約側にあるが、**保留の置き場だけは盤ごとに違う** —
  // 製品側 (STATUS_DESCRIPTION) は「前提情報の定義に従う」と盤に委ねており、
  // その答えを持っていたのがこの行だった。落とすと**委ねた先が空になる**
  "- 前提が足りず進められないものは status を変えず、summary に「◯◯待ちで保留」と現況を書く",
  "",
...
const CONTEXT_REFERENCE = [
  ...
  "",
  // #222 → #266: 「できなかったとき・やらないとき」の節は、参考にも雛形にも置かない。
  // **この参考の他の節は「どれか選ぶ」だが、ここだけ選択肢が無く雛形と同文だった** —
  // 保留・却下・誤登録の扱いはプロジェクトごとに変わるものではなく、製品の振る舞いそのもの。
  //
  // #222 では参考からだけ外し、雛形には残していた。**その理由は「節が欠けていれば
  // TEMPLATE_SECTIONS の検査で雛形が渡るから」だったが、それは配る側の都合**で、
  // 受け取った盤には「盤ごとに変わらないことが前提情報に書かれた」状態が残る。
  //
  // **読む側から見ると、同じことを両方に書くのは確認にならない** (#266):
  // LLMは前提情報が腐っていることに気づけない (#261 の `理由欄` は毎ターン読まれていた)。
  // 確認の価値は食い違ったときにしか生まれないのに、腐るコストは書いた瞬間から効く。
  //
  // **「食い違ったら前提情報が優先」は一般則ではない** (Codexレビュー 2周目 P3)。
  // 上の優先宣言が効くのは**この参考と現在の前提情報がぶつかったとき**で、
  // ほかに前提情報へ明示的に委ねているのは `STATUS_DESCRIPTION` の
  // 「status の置き場・列の意味」だけ。**そこでは腐った側が勝つ**が、
  // システムプロンプトの行動ルールとの衝突一般に前提情報が勝つ、とはどこにも書いていない。
  "## summary の書き方 (どれか選ぶ)",
