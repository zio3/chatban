...
/** キーの続きとみなす範囲。**マスク文字を列挙しない。**
 *
 * 最初は `[A-Za-z0-9_*.-]` と書いていたが、**上流が何でマスクするかは予想できない**
 * (Codexレビュー指摘)。実物のOpenAIは ASCII の `*` を並べてくるが、`…`(Unicode省略記号) や
 * `...`、`xxxx` を使う実装もありうる。列挙した文字クラスだと**そこで一致が止まり、
 * マスクの後ろに続く末尾数文字が残る** (`***…IgAA` の形)。
 *
 * そこで「空白まで」に変えた。**キーは空白を含まない**ので、いちど先頭を捕まえたら
 * トークンの終わりまで丸ごと伏せられる。マスク文字が何であっても効く */
const KEY_TAIL = "\\S*";
...
export function redactSecrets(text: string, secrets: (string | undefined | null)[]): string {
  ...
  // 設定に無いキーの保険。`sk-` + 8文字以上のキーらしい並びから、そのトークンの終わりまで。
  // 12文字以上の「キーらしい文字」を要求する形にしていたが、**マスクが早く始まると
  // 条件を満たさず素通りする** (`sk-abc***…` のような形)。頭の8文字で見切る
  return out.replace(new RegExp(`\\bsk-[A-Za-z0-9_-]{8,}${KEY_TAIL}`, "g"), mask);
}
