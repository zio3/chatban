...
export default function Board({
  ...
  //
  // **絞り込みを作り直すときは、ここに戻ってくること。方式を問わず区別が再び要る。**
  // 述語で絞る方式でも、CLAUDE.md の将来案 (押した瞬間のIDの集合で絞る) でも同じ —
  // **「あとから来たやつは全部勝ち」が防ぐのは、フィルタを押した後に作られたカードが埋もれること**で、
  // 押した時点で既にあって集合から外れた依存先は普通に隠れる。それが #41/#90 で踏んだ形そのもの。
  // (依存先IDを表示集合へ必ず足す仕様にすれば1本で足りるが、いまそんな仕様は決めていない)
