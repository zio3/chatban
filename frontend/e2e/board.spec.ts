...
test("LLMを直接呼ぶ口はGETに置かない (#180)", async () => {
  ...
  // 実測 (2026-08-20): 1回の全走で gpt-5.6-luna を **8回**呼んでいる (suggest×5 / chat×3)。
...
test("チャットに番号だけ打つと、LLMを呼ばずにそのタスクを開く (#197)", async ({ page }) => {
  ...
  // **LLMを呼んでいない。**番号ジャンプはLLMへ行く手前で横取りするので、応答待ちの
  // 「考え中…」が一度も出ない。出ないことがそのまま証拠になる。
  //
  // #219: 以前ここに「E2E環境には鍵が無いので失敗が出る」と書いていたが、**鍵はある**
  // (E2Eは backend/config.json をそのまま読む)。**LLMへ行けば失敗せずに成功して課金される**ので、
  // 「失敗が出る」を証拠にしてはいけない。見るのは失敗の有無ではなく「考え中…」の有無
...
test("存在しない番号は横取りせず、普通の発言としてLLMへ渡す (#197)", async ({ page }) => {
  ...
  //
  // #219: **このテストは有料のLLM呼び出しを1回起こす。**以前は「鍵が無いので結果は失敗でよい」と
  // 書いていたが誤り。実測 (2026-08-20) では成功していた: `#3 REQ "999999"` →
  // `#3 OK 1586ms rounds=1 tools=[] reply=37ch`。
  // 応答の中身は見ずに「パネルが開かないこと」だけを見るので、**呼び出し自体は無駄**。
  // それでも残しているのは、横取りしない経路がチャットへ流れることを端から端まで確かめたいため。
  // 消すか差し替えるかは別途判断する (呼び出しを止めるなら番号ジャンプの分岐だけを見る形にする)
  await expect(page.getByTestId("task-detail-panel")).toBeHidden();
