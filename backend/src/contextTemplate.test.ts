...
//
// **語で見るのはやめた** (Codexレビュー 2周目 P2)。1周目は4語の全面禁止、2周目は
// 「条件+手段+指示」の3群に増やしたが、どちらも**部分文字列の同居しか見ていない**ので
// 取りこぼしと誤検知が両方残った —「却下は rejected=true にして review へ移す」は
// 言い換えで素通りし、逆に「誤登録を削除する運用は、この盤では使わない」という
// **否定文が弾かれる**。語を足しても向きは変わらない。
//
// **#124 の3周目と同じ結論**なので、同じ形に寄せる —「してはいけないこと」を数える代わりに、
// **承認した全文をそのまま置く**。雛形を直したら必ずここが落ちるが、それは事故ではなく合図で、
// **人が期待値を書き直すときに「盤ごとに変わることか」を一度考える**のが目的。
// (落ちたときに何を考えるべきかは下のメッセージに書いてある)
const APPROVED_TEMPLATE = [
  ...
  // #266 で節ごと落とした「できなかったとき・やらないとき」から、**この1行だけ残した**。
  // 製品側 (STATUS_DESCRIPTION) が「status をどこに置くかは前提情報の定義に従う」と
  // 盤に委ねているので、消すと**委ねた先が空になる**
...
test("雛形は承認した文面のまま (#266)", () => {
  assert.deepEqual(
    CONTEXT_TEMPLATE.split("\n"),
    APPROVED_TEMPLATE,
    "雛形が承認した文面から変わっている。**足したものが「盤ごとに変わること」かを確かめること** — " +
      "却下・誤登録の扱いのように盤を変えても変わらないものは、書いても書き換える人がいない。" +
      "意図した変更なら、このテストの期待値も一緒に直す"
  );
});
