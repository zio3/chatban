 ...
 * 作り直すのは:
 *
 *   1. まだ無い。**初回のほか、読み込みが失敗したとき全般** — `dumpRequest` の try は
 *      `JSON.parse` だけでなく `readFileSync` も囲むので、権限やI/Oのエラーでも
 *      `prev` が `null` になる (2周目レビュー: 「JSONとして読めなかったとき」は狭かった)。
 *      **読めてさえいれば、中身が半端でも作り直さない** —
 *      `rounds` が空や `messageCount` 無しは、下の 3 で 0 と見なして足す側に入る
