...
// 「食い違ったら前提情報が優先」と契約に書いてあるので**腐った側が勝つ**。
//
// 見張るのは節の見出しではなく**語**にする。見出しを変えて同じ中身を戻せてしまうため。
test("雛形は保留・却下・誤登録の扱いを説明しない (#266)", () => {
  for (const word of ["保留", "rejected", "ゴミ箱", "誤登録"]) {
    assert.ok(
      !CONTEXT_TEMPLATE.includes(word),
      `雛形が「${word}」の扱いを書いている。盤ごとに変わらないので、書き換える人がいない。` +
