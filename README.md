...
- **キーは自分で置いてから頼む。** `~/.openai/apikey.txt` のようなファイルに1行で書いておき、設定には `apiKeyFile` でそのパスだけ書かせます。**あなたがキーを打ち込む必要が無くなる**のが効果です。ただし**隠せるわけではありません** — エージェントはそのファイルを読めるので、`cat` で中身を出せば会話の記録に載ります。「見せない」ではなく「**渡さなくて済む**」と考えてください
  ...
  キーをファイルに置く手間そのものも、**クリップボード経由でエージェントにやらせる**のが楽です。発行画面でキーをコピーしてから、こう言います:
  ...
  エージェントは `Get-Clipboard`(macOSなら `pbpaste`)の出力を**そのままファイルへ流す**ので、キーの値がターミナルにも会話にも出ません。**発行画面から一度もキーを目にせずに設置できます**。終わったらクリップボードは消しておいてください(残ったままだと、次に貼り付けた先へ入ります)。
...
| `EADDRINUSE` | 8787 か 5173 が空いていません。**まず残っているプロセスを探して、ツリーごと止めます**(Listenしているプロセスだけ落とすと `tsx watch` の親が残って再発します)。ポート自体を変えるのは下記のとおり一手間かかるので、後の手段です |
...
- **frontend (5173) を変える** — Viteの `--port`、backendの `CHATBAN_ALLOWED_ORIGINS`(**既定が `http://localhost:5173` なので、変えると自分のブラウザが弾かれます**)、`start-dev.ps1` 内のポート番号
