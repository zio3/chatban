...
function claimInTransaction(taskIds: number[], cardId: number): ClaimResult {
  ...
  // 外さないと、同じIDが2枚のカードの task_ids に載り (実測: stale:[1] fresh:[1])、
  // UIの件数が二重になるうえ、rollUpOldCards / compactArchive が古い索引から拾い直す
  // (Codexレビュー指摘)。**タスク側の持ち主は1つ**なので、索引もそれに合わせる
  const previousOwner = new Map<number, number | null>();
  ...
  for (const id of claimed) {
    const prev = previousOwner.get(id);
 ...
 *
 * 以前は `createSummaryCard()` が claim の外にあった。カードを作った直後、あるいは
 * 0件だったときの削除の前にプロセスが止まると、**空のカードが残る** (Codexレビュー指摘) —
 * 「プロセス停止からの回復」と「空カードを残さない」がこの札の要件なので、
